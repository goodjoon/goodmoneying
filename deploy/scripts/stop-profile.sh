...
print_compose_command() {
  ...
  printf 'ssh %s "cd '\''%s'\'' && DOCKER_CONFIG='\''%s'\'' PATH=%s docker compose --env-file '\''%s'\'' -f '\''%s'\'' stop"\n' \
    "$host" \
    "$base_dir" \
    "$docker_config" \
    "$REMOTE_DOCKER_PATH" \
...
run_compose_command() {
  ...
  ssh "$host" "cd '$base_dir' && DOCKER_CONFIG='$docker_config' PATH=/usr/local/bin:/Applications/Docker.app/Contents/Resources/bin:\$PATH docker compose --env-file '$compose_env' -f '$compose_file' stop"
}
